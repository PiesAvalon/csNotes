					 ...
					 SDL_Surface *dst, SDL_Rect *dstrect)
{
	...
	int sbpp = src->format->BitsPerPixel >> 3;
	int dbpp = dst->format->BitsPerPixel >> 3;
	assert(sbpp == dbpp);

	for (int row = 0; row < h; row++)
	{
		uint8_t *sp = src->pixels + (sy + row) * src->pitch + sx * sbpp;
		uint8_t *dp = dst->pixels + (dy + row) * dst->pitch + dx * dbpp;
		memcpy(dp, sp, w * sbpp);
	}
}
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color)
{
	...
	int bpp = dst->format->BitsPerPixel >> 3;
	for (int row = 0; row < h; row++)
	{
		uint8_t *line = dst->pixels + (y + row) * dst->pitch + x * bpp;
		memset(line, color, w * bpp);
	}

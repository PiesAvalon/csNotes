...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if (!overflow)
	{
		...
		{
			sig_grs++;
			if(sig_grs >> 24 == 1) // overflow after rounding
			{
				sig_grs = sig_grs >> 1;
				exp++;
				if (exp >= 0xff)
...
{
// printf("ref: 0x%08x + 0x%08x = ", a, b);
// printf("0x%08x\n", float_to_uint32((uint32_to_float(a) + uint32_to_float(b))));
	...
	if(fb.exponent == 0) // denormal
		shift++;
	if(fa.exponent == 0) // denormal
		shift--;
	...
	// printf("result: %f\n", uint32_to_float(internal_normalize(f.sign, exp_res, sig_res)));
	// printf("res bits: 0x%08x\n", internal_normalize(f.sign, exp_res, sig_res));
